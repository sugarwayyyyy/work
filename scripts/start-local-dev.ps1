...
Write-Host '[2/3] 啟動 backend 伺服器 http://127.0.0.1:8080 ...' -ForegroundColor Cyan
Start-Process -FilePath 'php' -ArgumentList '-S 127.0.0.1:8080' -WorkingDirectory $backendPath

Write-Host '[3/3] 啟動完成，請開啟下列網址：' -ForegroundColor Green
Write-Host '  前端: http://localhost:8000' -ForegroundColor Green
Write-Host '  後端: http://127.0.0.1:8080/api' -ForegroundColor Green
Write-Host ''
